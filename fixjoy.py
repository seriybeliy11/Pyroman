...
from pyrogram.types import Message, ReplyKeyboardMarkup, InlineKeyboardMarkup, InlineKeyboardButton, KeyboardButton, CallbackQuery, BotCommand, InputMediaPhoto, InputMediaVideo
        ...
        try:
            if message.media_group_id is None:
                ...
                if message.video_note:
                    ...
                    await client.send_video_note(chat_id=user_id, video_note=video_note)
            else:
                print('MEDIAG')
                n = message.sender_chat.username
                media_group = await client.get_media_group(chat_id=n, message_id=message.id)
                for media in media_group:
                    caption = media.caption if media.caption else ""
                    if media.video:
                        _media = InputMediaVideo(media.video.file_id, caption=caption)
                        lm.append(_media)
                    elif media.audio:
                        _media = await client.download_media(media.audio.file_id, in_memory=True)
                        lm.append(_media)
                    elif media.photo:
                        _media = InputMediaPhoto(media.photo.file_id, caption=caption)
                        lm.append(_media)
                    elif media.document:
            ...
            print(f'Errors: {e} ------ message {message.id} ---- caption {message.caption} --- {message}')
            continue
